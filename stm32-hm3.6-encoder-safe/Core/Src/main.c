...
{
    if (htim->Instance == ENCODER_TIMER.Instance) {
        app_isr_counter_callback();
    }
}
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
    if (GPIO_Pin == ENCODER_BTN_Pin) {
        app_isr_button_callback();
    }
}

void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
    if (htim->Instance == BUZZER_TIMER.Instance) {
        buzzer_periodElapsed_callback();
    }
}
