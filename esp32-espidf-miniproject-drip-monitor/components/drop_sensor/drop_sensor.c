...
void drop_sensor_init(void) {
    adc_oneshot_unit_init_cfg_t unit_cfg = { .unit_id = DROP_ADC_UNIT };
    ...
    ESP_ERROR_CHECK(adc_oneshot_config_channel(s_adc_handle, DROP_ADC_CHANNEL, &chan_cfg));
}

uint16_t drop_sensor_read_raw(void) {
    int raw = 0;
    ESP_ERROR_CHECK(adc_oneshot_read(s_adc_handle, DROP_ADC_CHANNEL, &raw));
    return (uint16_t)raw;
}
